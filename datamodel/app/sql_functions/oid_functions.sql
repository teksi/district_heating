...
CREATE OR REPLACE FUNCTION tdh_app.generate_oid(schema_name text, table_name text)
  RETURNS text AS
$BODY$
DECLARE
  myrec_prefix record;
...
BEGIN
  ...
  SELECT tdh_app.generate_oid_postfix(schema_name,table_name) as _postfix INTO myrec_seq;
  IF NOT FOUND THEN
...
BEGIN
	BEGIN
	   FOR tbl_name,rgx IN (
		...
		SELECT tdh_app.base36_to_int(RIGHT(obj_id, 6)) as seqs
		FROM tdh_od.%1$I
...
CREATE OR REPLACE FUNCTION tdh_app.generate_oid_postfix(_schema text, _table_name text)
...
BEGIN
	...
	ELSIF val < 559744029 THEN --  559744029 is 999999 in base36, by skipping to this bigint we avoid purely numeric base36 varchars
		...
		RETURN tdh_app.generate_oid_postfix(_schema, _table_name);
	ELSE
		WHILE val != 0 LOOP
...
CREATE OR REPLACE FUNCTION tdh_app.base36_to_int(base36_str text)
RETURNS bigint
LANGUAGE plpgsql
AS $BODY$
DECLARE
