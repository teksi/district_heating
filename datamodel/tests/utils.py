...
class DbTestBase:
    ...
    @classmethod
    def select(cls, table, obj_id, schema="tdh_app"):
        try:
    ...
    @classmethod
    def insert(cls, table, row, schema="tdh_app"):
        cur = cls.conn.cursor()
        cols = ", ".join(row.keys())
    ...
    @classmethod
    def update(cls, table, row, obj_id, schema="tdh_app"):
        cur = cls.conn.cursor()
    ...
    @classmethod
    def delete(cls, table, obj_id, schema="tdh_app"):
        cur = cls.conn.cursor()
        cur.execute(f"DELETE FROM {schema}.{table} WHERE obj_id=%s", [obj_id])

    def insert_check(self, table, row, expected_row=None, schema="tdh_app"):
    ...
    def update_check(self, table, row, obj_id, schema="tdh_app"):
        self.update(table, row, obj_id, schema)
        result = self.select(table, obj_id, schema)
        self.check_result(row, result, table, "update", schema)

    def check_result(self, expected, result, table, test_name, schema="tdh_app"):
