TDH-project properties and settings
===================================

Project Properties
------------------
There are several relevant properties in a TDH-project:
...
Relations
^^^^^^^^^^^^
Relations are necessary for the Feature Attributes Windows to reference to connected tables (e.g. you can edit connected maintenance_events in Feature_Attribut window in vw_tdh_reach).
...
Variables
^^^^^^^^^^
The TDH-project uses project variables to simplify some of QGIS-functionalities. These approaches are not TDH specific and can be used also in other QGIS-projects.

In the project used variables have names that start with "tdh".

.. figure:: images/properties_variables.jpg

**Variables for label translations**

TDH uses 4 variables to translate the labeling of manholes into the desired language. Without changing the variables, the levels are labeled with "english" letters: "C" for cover level, "I" for Input level, "O" for Output level, "B" for Bottom level. In a german project, you will change these variables to "D" (Deckel), "E" (Einlauf), "A" (Auslauf) and "S" (Sohle) to get the expected labels.

Background: the database uses always the "english" letters to write the _*_label fields in vw_tdh_wastewater_structure. In the label-definition of vw_tdh_wastewater_structure is a "replace" formula, that change the "english" letter to the letter defined in the project variables

'concat(_label, replace(_cover_label,'C',@tdh_cover_prefix), replace(_bottom_label,'B',@tdh_bottom_prefix), replace(_input_label,'I',@tdh_input_prefix), replace(_output_label,'O',@tdh_output_prefix))'
...
Example in vw_tdh_reach:

.. figure:: images/properties_variables_example1.jpg

Styles/Project Colors
^^^^^^^^^^^^^^^^^^^^^
The color for mixed_wastewater (as example) is used in vw_tdh_reach symbols and labels, in vw_tdh_wastewater_structures symbols and labels. To change the color (just a different shade of purple) is a lot of work and may result in errors, if you do not use project colors.
In the TDH-project are project colors defined for all value of valuelist
...
Map Themes
----------
There are two map themes defined in the TDH-project as an invitation to use this great feature of QGIS to toggle between different layer-styles.
...
Layer Order
-----------
TDH uses a lot of layers. Therefore it's a good idea to have the layer organised in a logical way.
